		...
		ostream &out)					// output stream
{
	...
	out << "Me: " << std::hex << (int) this % 0x10000;
	out << "/Parent: " << std::hex << (int) parent % 0x10000;
		...
		ostream &out)					// output stream
{
	...
	if (with_pts) {						// print point coordinates
		...
		for (int i = 0; i < n_pts; i++) {
			...
			out << ", Addr: " << std::hex << (int)leafAddr[i] % 0x10000;
			out << "\n";
		}
	}
		...
		ANNidxArray pi)					// point indices
{
	...
	if (n<=0) {
		//searchStart = leafAddr = NULL;
		leafAddr = NULL;
		distances = NULL;
	} else {
		leafAddr = new ANNkd_ptr[n];
		//searchStart = new ANNkd_ptr[n];
		distances = new compressed_matrix<double>(n_pts,n_pts);
	}


	bnd_box_lo = bnd_box_hi = NULL;		// bounding box is nonexistent
	if (KD_TRIVIAL == NULL)				// no trivial leaf node yet?
		KD_TRIVIAL = new ANNkd_leaf(0, IDX_TRIVIAL, leafAddr);//,searchStart);	// allocate it
	...
	ANNkd_ptr*			&leafAddr)		// pointer to leaves
{
	if (n <= bsp) {						// n small, make a leaf node
		if (n == 0)						// empty leaf node
			...
			return new ANNkd_leaf(n, pidx, leafAddr);//,searchStart);
		}
	}
		...
		lo = rkd_tree(					// build left subtree
				...
				dim, bsp, bnd_box, splitter,leafAddr);//,searchStart);
	...
	ANNsplitRule		split)			// splitting method
{
	...
	case ANN_KD_STD:					// standard kd-splitting rule
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, kd_split,leafAddr);//, searchStart);
		break;
	case ANN_KD_MIDPT:					// midpoint split
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, midpt_split,leafAddr);//,searchStart);
		break;
	case ANN_KD_FAIR:					// fair split
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, fair_split,leafAddr);//,searchStart);
		break;
	case ANN_KD_SUGGEST:				// best (in our opinion)
	case ANN_KD_SL_MIDPT:				// sliding midpoint split
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, sl_midpt_split,leafAddr);//,searchStart);
		break;
	case ANN_KD_SL_FAIR:				// sliding fair split
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, sl_fair_split,leafAddr);//,searchStart);
		break;
	default:
