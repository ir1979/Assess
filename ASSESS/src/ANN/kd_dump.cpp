...
static ANNkd_ptr annReadTree(			// read tree-part of dump file
	...
	//,ANNkd_ptr*			&searchStart			// search start pointer
	);
	...
	ANNpoint			&the_bnd_box_hi)		// high bounding point (ret'd)
{
	...
	if (strcmp(str, "tree") == 0) {				// tree section
		...
		ANNkd_ptr* the_leafAddr = new ANNkd_ptr[the_n_pts];
		//ANNkd_ptr* the_searchStart = new ANNkd_ptr[the_n_pts];
		//compressed_matrix<double> *distances = new compressed_matrix<double>(the_n_pts, the_n_pts);

												// read the tree and indices
		the_root = annReadTree(in, tree_type, the_pidx, next_idx, the_leafAddr);//, the_searchStart);
...
static ANNkd_ptr annReadTree(
	...
	int					&next_idx,				// next index (modified)
	ANNkd_ptr*			&leafAddr				// pointer to leaves
	//,ANNkd_ptr*			&searchStart			// pointer to start/continue the search
	)
{
	...
	if (strcmp(tag, "leaf") == 0) {				// leaf node
		...
		return new ANNkd_leaf(n_pts, &the_pidx[old_idx], leafAddr);//, searchStart);
	}
	//------------------------------------------------------------------
	...
	//------------------------------------------------------------------
		...
		ANNkd_ptr lc = annReadTree(in, tree_type, the_pidx, next_idx, leafAddr);//, searchStart);
		ANNkd_ptr hc = annReadTree(in, tree_type, the_pidx, next_idx, leafAddr);//, searchStart);
