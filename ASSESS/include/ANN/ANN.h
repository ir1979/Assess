...
#define DETAILS_LEVEL 1
#define EPSILON1 1e-6
#define EPSILON2 1e-10
#define myInf 1e15
//#define DEBUG
//#define __SHOW_PROGRESS__
//#define __ADD_LOG__
...
#include <ANN/checkActiveClass2.h>
#include <ANN/checkActiveClass3.h>
...
protected:
	...
	ANNkd_ptr		*leafAddr;			// pointer to leaf nodes
	//ANNkd_ptr		*searchStart;		// pointer to start/continue the search from bottom to up
	compressed_matrix<double> 
					*distances;			// sparse matirix to save calculated distances
...
public:
	...
	int annkSearchContinuous(
		...
		ANNdistArray		dd,				// the approximate nearest neighbor
		double				eps,			// the error bound
		checkActiveClass3	*checkActive);	// 

	int annkSearchContinuous_BottomUp(
		int					nodeIdx,		// the index of query point (between all nodes in the kdtree)
		int					k,				// number of near neighbors to return
		ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
		ANNdistArray		dd,				// the approximate nearest neighbor
		double				eps,			// the error bound
	...
	int annkSearchContinuous_BottomUp(
		...
		ANNdistArray		dd,				// the approximate nearest neighbor
		double				eps,			// the error bound
		checkActiveClass3	*checkActive);	// check to be active 

	int annkSearch_BottomUp(
		int					nodeIdx,		// the index of query point (between all nodes in the kdtree)
		int					k,				// number of near neighbors to return
		ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
		ANNdistArray		dd,				// the approximate nearest neighbor
	...
	bool annDeleteNode_BottomUp(
		int					nodeIdx);		// the index of the node to be deleted (starting from the leave node)
