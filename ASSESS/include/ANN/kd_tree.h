...
public:
	...
	virtual void ann_searchContinuous(ANNdist box_dist, double minDist) = 0; // search with minimum distance
	virtual void ann_searchContinuous(ANNdist,checkActiveClass1*) = 0;
	virtual void ann_searchContinuous(ANNdist,checkActiveClass2*) = 0;
	virtual void ann_searchContinuous(ANNdist,checkActiveClass3*) = 0;
	virtual void ann_searchContinuous(ANNdist,checkActiveClass3*, compressed_matrix<double>*) = 0;
	virtual void ann_searchContinuous_BottomUp(ANNdist,checkActiveClass2*,ANNkd_ptr, int) = 0;
	virtual void ann_searchContinuous_BottomUp(ANNdist,checkActiveClass3*,ANNkd_ptr, int) = 0;
	virtual void ann_search_BottomUp(ANNdist,ANNkd_ptr, int) = 0;
	virtual bool annDeleteNode(int, ANNdist) = 0;
	virtual bool annDeleteNode_BottomUp(int) = 0;
...
public:
	ANNkd_leaf(							// constructor
		...
		ANNkd_ptr*		&leafAddr)		// leaf index
		{
			n_pts		= n;			// number of points in bucket
			bkt			= b;			// the bucket

			if (n_pts==1) 				// most usual case
					leafAddr[bkt[0]] = this;
			else 
				for (int idx=0;idx<n_pts;idx++)
					leafAddr[bkt[idx]] = this;
		}
	...
	virtual void ann_searchContinuous(ANNdist box_dist, double minDist); // search with minimum distance
	virtual void ann_searchContinuous(ANNdist box_dist, checkActiveClass1 *checkActive);
	virtual void ann_searchContinuous(ANNdist box_dist, checkActiveClass2 *checkActive);
	virtual void ann_searchContinuous(ANNdist box_dist, checkActiveClass3 *checkActive);
	virtual void ann_searchContinuous(ANNdist box_dist, checkActiveClass3 *checkActive, compressed_matrix <double>*);
	virtual void ann_searchContinuous_BottomUp(ANNdist box_dist, checkActiveClass2 *checkActive,ANNkd_ptr, int);
	virtual void ann_searchContinuous_BottomUp(ANNdist box_dist, checkActiveClass3 *checkActive,ANNkd_ptr, int);
	virtual void ann_search_BottomUp(ANNdist box_dist,ANNkd_ptr, int);
	virtual bool annDeleteNode(int,ANNdist);
	virtual bool annDeleteNode_BottomUp(int nodeIdx);
...
public:
	...
	~ANNkd_split()						// destructor
		{
			//cout << "The split node " << std::hex << (int)this%0x10000 << std::dec << " is deleted." << endl;
	...
	virtual void ann_searchContinuous_BottomUp(ANNdist box_dist, checkActiveClass3 *checkActive,ANNkd_ptr, int);
	virtual void ann_search_BottomUp(ANNdist box_dist,ANNkd_ptr, int);
	virtual bool annDeleteNode(int nodeIdx, ANNdist box_dist);
	virtual bool annDeleteNode_BottomUp(int nodeIdx);
	virtual void ann_pri_search(ANNdist);		// priority search
	virtual void ann_FR_search(ANNdist);		// fixed-radius search
};
...
ANNkd_ptr rkd_tree(				// recursive construction of kd-tree
	...
	ANNkd_ptr*			&leafAddr 		// pointer to leaves
	//,ANNkd_ptr*			&searchStart	// pointer to start/continue the search from button to up
	);
